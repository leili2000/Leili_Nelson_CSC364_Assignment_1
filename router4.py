...
def processing_thread(connection, ip, port, forwarding_table, default_gateway_port, max_buffer_size=5120):
    ...
    while True:
        ...
        ## if ...:
        if new_ttl > 0 and sending_port == "8005":
            print("sending packet", new_packet, "to Router 5")
            ## ...
            soc5.sendall(bytes(new_packet, encoding='utf-8'))
            write_to_file("output\\sent_by_router_4.txt", new_packet, send_to_router = "5")
        ## elif ...:
        elif new_ttl > 0 and sending_port == "8006":
            print("sending packet", new_packet, "to Router 6")
            ## ...
            soc6.sendall(bytes(new_packet, encoding='utf-8'))
...
# 1. Connect to the appropriate sending ports (based on the network topology diagram).
## ...
soc5 = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
print("Router 5 socket created")
soc5.connect((host, port5))
## ...
soc6 = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
print("Router 6 socket created")
soc6.connect((host, port6))
start_server()
